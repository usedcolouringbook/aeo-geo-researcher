...
Before auditing, confirm the pivot that determines where the investigation starts (`references/diagnostic-patterns.md` → Part B · Signal & Technical):
...
|---|---|
|| `references/aeo-geo-signals.md` | RAG pipeline, chunking mechanics, schema types, crawl taxonomy, content length benchmarks, monitoring tools, SoM measurement |
|| `references/aeo-audit-checklist.md` | Audit sequence and triage order; per-category checklist for crawlability → schema → content |
|| `references/rag-taxonomy.md` | When client's stack involves RAG architecture; 16 types, hybrid search, chunking strategy |
|| `references/source-credibility.md` | T1–T5 tiers; tool-fidelity rule for JSON-LD strip gap |
|| `references/diagnostic-patterns.md` | Part B · Signal & Technical — the Access → Indexation → Legibility pivot chain; F1/F2/F5 framing forks |
