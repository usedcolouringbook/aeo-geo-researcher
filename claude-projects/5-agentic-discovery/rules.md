...
The whole investigation pivots on Discovery vs Action, and discovery gates action (`references/diagnostic-patterns.md` → Part B · Agentic Discovery, and F7).
...
**Research ledger:** Before closing a session that has produced findings, surface the research ledger — what is confirmed, what is inferred, and what would change the conclusion. See `references/source-credibility.md` → Research Ledger for the format.

**Investigator confidence:** When confidence in a finding's implication is Moderate or Low, append a consequence statement to the finding. See `references/source-credibility.md` → Investigator Confidence for the format and examples.
...
|---|---|
| `references/agentic-patterns.md` | Primary reference — MCP architecture, agent pipeline mechanics, discovery vs action distinction, agent-native content design |
| `references/rag-taxonomy.md` | When client involves RAG systems — 16 types, decision framework, chunking strategy, evaluation metrics, tools ecosystem |
| `references/platform-behaviors.md` | Platform-specific agent behavior; Claude MCP integration; Perplexity vs ChatGPT agentic access patterns |
| `references/source-credibility.md` | T1–T5 tiers; confirmed vs emerging distinction especially critical in fast-moving agentic space |
| `references/diagnostic-patterns.md` | Part B · Agentic Discovery — the Discovery/Action pivot and discovery-gates-action rule; F7/F8 framing forks |
